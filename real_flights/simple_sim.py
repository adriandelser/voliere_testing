...
class CaseMaker(Observer):
    ...
    def fly(self, case:Case):
        ...
        try:
            ...
            for i in range(4):
                # print(self.swarm.tellos[0].get_position_enu())
                headings = [(key, vehicle.heading, vehicle.get_position_enu()) for key, vehicle in vehicles.items()]
                ids = [tello.ac_id for tello in self.swarm.tellos]
                print(headings, ids)
                # print(vehicles["69"].heading)
            ...
            #         self.swarm.tellos[idx].fly_to_enu(INIT_XYZS[idx], heading=0)

            while time.time()-starttime < 5:
                
                # for idx, _ in enumerate(id_list):

                # self.swarm.tellos[0].fly_to_enu(convert_coords(INIT_XYZS[0]), heading=0)
                # velocity[2] = 0 #ensure z velocity is 0
                desired = [-0.2,-0.4,0]
                # tello_input_v = [-desired[1], desired[0],0]
                # tello_input_v = convert_coords(desired)
                # self.swarm.tellos[0].send_velocity_enu(desired, heading=0)
                for idx, tello in enumerate(self.swarm.tellos):
                    tello.fly_to_enu(INIT_XYZS[tello.ac_id], heading=0)
                # print(self.swarm.tellos[0].get_velocity_enu())
                # pass
